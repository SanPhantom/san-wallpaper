...
const AutoSizerMasonry = <T extends Record<string, any>>(props: AutoSizerMasonryProps<T>) => {
  const { target, ...masonryProps } = props;

  const height = target.current?.getBoundingClientRect().height ?? 0;

  return height ? (
    <AutoSizer disableHeight scrollTop={props.scrollTop} height={height} overscanByPixels={1500}>
      {({ width }) => {
        return <CommonMasonry width={width} height={height * 2} {...masonryProps} />;
      }}
    </AutoSizer>
  ) : null;
};
